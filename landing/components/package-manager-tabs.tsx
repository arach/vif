          ...
          <TabsList className="inline-flex bg-white/90 backdrop-blur-sm border border-slate-200/80 rounded-2xl p-1 shadow-lg shadow-slate-200/50">
            {Object.entries(packageManagers).map(([key, pm], index) => (
              <TabsTrigger
                key={key}
                value={key}
                className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-1.5 sm:py-3 rounded-xl font-medium text-xs sm:text-sm relative z-10 transition-all duration-300 ease-in-out before:absolute before:inset-0 before:rounded-xl before:transition-all before:duration-300 before:ease-in-out data-[state=active]:before:bg-white data-[state=active]:before:shadow-md data-[state=active]:before:border data-[state=active]:text-violet-700 data-[state=active]:before:border-violet-200/50 data-[state=active]:before:shadow-violet-200/30 data-[state=inactive]:before:bg-transparent data-[state=inactive]:before:shadow-none data-[state=inactive]:before:border-transparent data-[state=inactive]:text-slate-600 data-[state=inactive]:hover:text-slate-800 data-[state=inactive]:hover:before:bg-slate-50/50 ${
