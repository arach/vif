 ...
 */
async function checkServerStatus(): Promise<{ connected: boolean; agentReady: boolean }> {
  const port = 7850;
 ...
 */
async function startServerBackground(verbose = false): Promise<void> {
 ...
 */
async function stopServer(verbose = false): Promise<void> {
  const log = verbose ? console.log.bind(console) : () => {};
